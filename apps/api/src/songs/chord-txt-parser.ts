...
function classifyLine(line: string): ParsedSectionLine {
...
function parseMetadata(lines: string[]): ParsedChordTxt["metadata"] {
...
export function parseChordTxt(rawInput: string): ParsedChordTxt {
  ...
  let currentSection: ParsedSection = { name: "Sem seção", lines: [] };
