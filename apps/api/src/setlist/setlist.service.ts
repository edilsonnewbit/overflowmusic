...
export class SetlistService {
  ...
  async addItem(eventId: string, input: CreateSetlistItemInput, actor: SetlistActor) {
  ...
  async updateItem(eventId: string, itemId: string, input: UpdateSetlistItemInput, actor: SetlistActor) {
    const setlist = await this.getOrCreateSetlist(eventId);
  ...
  async removeItem(eventId: string, itemId: string, actor: SetlistActor) {
    const setlist = await this.getOrCreateSetlist(eventId);
    const item = await this.prisma.setlistItem.findFirst({
      where: { id: itemId, setlistId: setlist.id },
    });
  ...
  async reorder(eventId: string, input: ReorderInput, actor: SetlistActor) {
    const setlist = await this.getOrCreateSetlist(eventId);
  ...
  async getEventLogs(
    ...
    const setlist = await this.prisma.setlist.findUnique({ where: { eventId }, select: { id: true } });
    ...
    const [logs, total] = await Promise.all([
      this.prisma.setlistLog.findMany({ where, orderBy: { createdAt: "desc" }, take: limit, skip }),
